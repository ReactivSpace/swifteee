angular.module('App.CTCCtrl', [])
.controller('CTCCtrl', function($scope, $cordovaCamera, $cordovaFile, $firebaseArray, $ionicPlatform, upladingLoader, $cordovaToast){
    var id = localStorage.getItem("doc_id");
    $scope.images = [];
    var ref = firebase.database().ref().child("profiles/"+id+"/documents");
    var find = ref.orderByChild('name').equalTo('CTC');
    var uploaded;
    ...
    $scope.newUpload = function(){
      $cordovaCamera.getPicture(options).then(function(imageData) {
          upladingLoader.show();
          sync.$add({image: imageData, name: 'CTC', status: 1}).then(function(success){
